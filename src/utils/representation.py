def entity_representation(subject: str, object: str, sentence: str, method: str = None) -> str:
    ...
    subject_entity = subject[:-1].split(",")[3].split(":")[1]
    object_entity = object[:-1].split(",")[3].split(":")[1]

    sub_start_idx = int(subject.split(",")[1].split(":")[1])
    sub_end_idx = int(subject.split(",")[2].split(":")[1])
    subject = subject.split(",")[0].split(":")[1]

    obj_start_idx = int(object.split(",")[1].split(":")[1])
    obj_end_idx = int(object.split(",")[2].split(":")[1])
